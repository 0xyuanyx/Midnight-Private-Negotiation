...
- 한도를 잠근 시점에 상대 역할 또는 상대 가격 커밋이 아직 준비되지 않은 경우에만 대기 로그를 표시한다. 이미 준비된 상태라면 대기 로그 없이 완료 이벤트를 즉시 동기화한다.
...
- 최대 10라운드는 내부 종료 조건이며 라운드 번호, 폐기 후보 수, 재시도 횟수를 어떤 패널에도 표시하지 않는다.
- 지갑 주소, 트랜잭션 해시, 블록 번호는 메인 3패널 로그에 표시하지 않는다. 공개 네트워크 검증 자료가 필요하면 별도 익스플로러 화면에서 다룬다.
...
- 일반 문장은 `#FFFFFF`, 시간은 `#A8A8A8`이다. `비공개 상태`, `비공개 협상`, `금액 비공개` 토큰만 `#D0B36C`로 표시한다. `공개하지 않고`와 가격 커밋 문구는 흰색으로 유지한다.
- 화면에 표시되는 증명, `OPEN`, `AUTHORIZED` 같은 프로토콜 토큰만 `#9A9AFF`로 표시한다. 내부 `createDeal`, `joinDeal` 이벤트는 브라우저 스트림에서 제외한다.
- Observer의 `거래 확정`, `SETTLED`, 최종 금액 토큰과 Buyer·Seller의 합의 금액 토큰은 `#9FB8A3`로 표시한다. 문장 전체를 성공색으로 칠하지 않는다.
...
[09:41:08] 구매자 가격 커밋을 생성했습니다.
[09:41:09] 판매자 가격 커밋 등록을 기다리고 있습니다. ⠋
[09:41:12] 판매자 가격 커밋이 등록되었습니다.
[09:41:13] 협상을 시작합니다. ⠋
...
진행 중 로그는 한 번 출력되면 자리를 유지한다. 다음 상태가 새 행으로 추가될 때 기존 행의 스피너만 정지한다. 폐기된 GPT 후보, 재요청, 제안 가격, 상대 응답 원문, `(1/10)` 같은 라운드 표시는 출력하지 않는다. 대기 로그를 만들기 전에는 상대 입장과 가격 커밋 상태를 먼저 확인한다.

### Observer 공개 상태 로그

```text
[09:41:13] 거래 개시 · OPEN
[09:41:24] 가격 조건 승인 · AUTHORIZED · 금액 비공개
[09:41:31] 거래 확정 · SETTLED · 100,000 KRW
```
...
- 역할 구분색은 헤더 텍스트에만 제한한다. Buyer는 clear blue-gray `#A9C2E6`, Seller는 clear sand-gold `#D6B879`, Observer는 흰색을 사용하며 로그 의미 색상과 상태점에는 적용하지 않는다.
