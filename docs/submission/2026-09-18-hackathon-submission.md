...
2026-09-24에 최종 코드(`ef749ee` 기준)로 로컬 Midnight 체인을 실행하며 113초 영상을 새로 만들었다. 1600×900, WebM(VP8), 10fps. 구성은 제목 → 문제 → 시연 조건 → 시연(상품 코드 입장, 각자 한도 입력, `OPEN → AUTHORIZED → SETTLED · 금액 비공개`, 양측 합의 가격 확인·증빙 보관·정리) → 앱 종료 후 `npm run evidence -- verify` 실제 출력 → 마무리다.

- 실제 데모 화면을 headless Chromium으로 조작하며 프레임 단위로 캡처했다. 앱 화면의 로그는 런타임이 보낸 그대로이며 합성하지 않았다.
- 체인 대기 구간만 20배속 타임랩스로 줄였고, 해당 구간 오른쪽 아래에 "대기 구간 20배속"을 표시했다. 입력과 결과 구간은 실제 속도다.
- 화면 아래 설명 자막과 제목·설명 카드는 편집으로 넣은 것이다. 증빙 확인 장면의 터미널 내용은 녹화 직후 실제로 실행한 명령의 출력을 그대로 옮겼다.
- 협상 후보는 mock 생성기로 만들었고, 시연 조건 카드에 그렇게 표시했다. 모든 금액은 가상 시연 값이다.
- 2026-09-18 영상은 최종가 공개 버전이라 더 이상 쓰지 않는다.
