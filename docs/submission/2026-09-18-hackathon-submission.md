...
- **합의 가격이 공개 기준가로 고정되지 않는다.** 양측 사다리가 항상 공개 기준가에서 만나면 기준가를 아는 사람이 `SETTLED`만 보고 합의 가격을 알 수 있다. 그래서 Buyer와 Seller가 거래마다 새로 만든 임시 X25519 공유 비밀에서 비공개 오프셋을 각자 계산해 두 사다리를 함께 옮긴다. 오프셋은 한도와 무관하고 AI·Controller·Relay·원장에 전달되지 않는다. AI에는 오프셋을 뺀 값만 보내고, 돌아온 후보는 실제 가격으로 되돌린 뒤 실제 한도로 다시 검사한다.
...
- 거래 시점, 수락·중단, 당사자가 공개한 정보에 따른 추론은 남는다. 오프셋은 가격 결정 규칙의 보완이며 공정한 시장 가격이나 모든 추론으로부터의 완전한 비밀성을 보장하지 않는다.
