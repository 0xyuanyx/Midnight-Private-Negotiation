...
2026-09-26에 거래별 가격 오프셋을 적용한 코드(`bb9490f`)로 로컬 Midnight 체인과 OpenAI 모드를 실행하며 웹 화면만 29초로 녹화했다. 1600×900, WebM(VP8), 10fps. 상품 코드 입장, 각자 한도 입력(Buyer 110,000,000 / Seller 95,000,000), `OPEN → AUTHORIZED → SETTLED · 금액 비공개`, 양측 합의 가격(이번 실행 98,932,750 KRW) 확인과 증빙 보관·정리까지 담았다.
