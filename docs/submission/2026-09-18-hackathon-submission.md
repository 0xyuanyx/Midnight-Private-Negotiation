...
- **합의 가격이 공개 기준가로 고정되지 않는다.** 양측 사다리가 항상 공개 기준가에서 만나면 기준가를 아는 사람이 `SETTLED`만 보고 합의 가격을 알 수 있다. 그래서 Buyer와 Seller가 거래마다 새로 만든 임시 X25519 공유 비밀에서 비공개 오프셋(기준가의 ±3%, 250원 단위)을 각자 계산해 두 사다리를 함께 옮긴다. 오프셋은 한도와 무관하고 AI·Controller·Relay·원장에 전달되지 않는다. AI에는 오프셋을 뺀 값만 보내고, 돌아온 후보는 실제 가격으로 되돌린 뒤 실제 한도로 다시 검사한다.
...
- 루트 테스트 74/74, 웹 테스트 8/8, `typecheck` 통과 (2026-09-26, 거래별 가격 오프셋 적용 후). 이전 기록: 루트 62/62, 계약 테스트 8/8 (2026-09-23). 계약 테스트에는 제3자의 참여·승인·정산·취소 거절과 공개 ledger·transcript의 가격 부재 검사가 포함된다.
- 공개 원격을 새로 클론해 설치·컴파일·전체 테스트가 통과하는 것을 확인했다 (2026-09-18).
- 2026-09-26 로컬 체인·실제 OpenAI 모드에서 성공 `OPEN → AUTHORIZED → SETTLED · 금액 비공개`, 양측 합의 가격 101,333,250 KRW 일치, 앱 종료 후 증빙 복호화·온체인 대조 성공. Indexer의 계약 트랜잭션 4건 원문과 계약 상태에 합의 가격·양측 한도의 인코딩 값이 없음을 확인했다.
...
- 합의 가격은 공개 기준가의 ±3% 범위라는 사실까지는 알려진다. 거래 시점, 수락·중단, 당사자가 공개한 정보에 따른 추론은 남는다. 오프셋은 가격 결정 규칙의 보완이며 공정한 시장 가격이나 모든 추론으로부터의 완전한 비밀성을 보장하지 않는다.
...
https://youtu.be/38m1soEahMs

2026-09-26에 거래별 가격 오프셋을 적용한 코드(`bb9490f`)로 로컬 Midnight 체인과 OpenAI 모드를 실행하며 웹 화면만 54초로 녹화했다. 1600×900, WebM(VP8), 10fps. 앞부분은 성공 시나리오로 상품 코드 입장, 각자 한도 입력(Buyer 110,000,000 / Seller 95,000,000), `OPEN → AUTHORIZED → SETTLED · 금액 비공개`, 양측 합의 가격(이번 실행 98,932,750 KRW) 확인과 증빙 보관·정리까지 담았다. 뒷부분은 초기화 뒤 결렬 시나리오(Buyer 90,000,000 / Seller 95,000,000)로 `OPEN → CANCELLED · 공개된 금액 없음`과 중간 기록 정리를 담았다. 두 시나리오는 따로 녹화해 이어 붙였다.

- headless Chromium으로 실제 데모 화면을 조작하며 프레임 단위로 캡처했다. 앱 로그는 런타임이 보낸 그대로이며 자막·설명 카드는 없다.
- 체인·협상 대기 구간만 20배속으로 줄였고, 해당 구간 오른쪽 아래에 "체인·협상 대기 20배속"을 표시했다. 나머지 구간은 실제 속도다.
- 합의 가격은 실행 결과 그대로다. 같은 조건의 다른 실행에서는 101,333,250 KRW, 102,917,500 KRW로 성사됐다.
- 2026-09-24 영상(합의 가격이 항상 공개 기준가 100,000,000 KRW로 나오던 버전)과 2026-09-18 영상(최종가 공개 버전)은 더 이상 쓰지 않는다.
